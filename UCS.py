    ...
    def ucs(self, gui):
        ...
        while not self.open_set.empty():
            ...
            for direction in neighbors:
                cost = 1
                if (direction == neighbors[0] or
                    direction == neighbors[2] or
                    direction == neighbors[4] or
                    direction == neighbors[6]):
                    cost = 1.5
                    print(cost)
                neighbor = (current[0] + direction[0], current[1] + direction[1])
                if not self.isValid(neighbor):
                    continue
                if neighbor in self.close_set and self.g_score[current] + cost >= self.g_score[neighbor]:
                    continue
                gui.updateMap(neighbor, NEIGHBOR_COLOR)
                if neighbor not in self.g_score or self.g_score[current] + cost < self.g_score[neighbor]:
                    self.came_from[neighbor] = current
                    self.g_score[neighbor] = self.g_score[current] + cost
                    self.open_set.put(neighbor, self.g_score[neighbor])
                    #print(neighbor[1], self.map_height - neighbor[0] - 1, self.g_score[neighbor])
        return []

    def runUCS(self, gui, input_name = 'input1.txt', output_name = 'output1.txt'):
        global sum_delay
        sum_delay = 0
        ...
        if self.minStep == -1:
            ...
            Notification().alert("Time: ",
                                 repr(time.clock() - START_TIME - sum_delay) + "s\nStep: " + repr(self.minStep - 1) + "\nCost: " + repr(self.g_score.get(self.goal)))